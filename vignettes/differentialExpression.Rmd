...
First, we load the `adaptest` package and the (included) `illuminaData` data set:

```{r setup_data}
library(dplyr)
library(adaptest)
library(adaptestData)
data(illuminaData)
...
With the `illuminaData` data set below, we
...
`modtest_ic`; (3) `limmaOut`, an empty slot meant to hold the output of
`limma::lmFit`, after a later call to `modtest_ic`; and (4) `tmleOut`, a data
...
around `limma::lmFit` and `limma::topTable` that outputs a `adaptest` object
with the slots described above completely filled in. The `modtest_ic` function
requires as input a `adaptest` object containing a data frame in the `tmleOut`
...
```{r limmaTMLE_eval}
limmaTMLEout <- modtest_ic(adaptest = biomarkerTMLEout)
```
