...
#'         Hubbard, with contributions from Nima Hejazi.
#' @export
data_adapt_multi_test <- function(Y, A, W = NULL, n.top, n.fold,
  ...
  if (parallel) {
    ...
    if (detectCores() == 1) {
      stop("argument parallel is set to TRUE, but multiple cores have not been detected")
    }
  }
  ...
  # ============================================================================
  # boxplot.matrix(top.rank.all.folds
