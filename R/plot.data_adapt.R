...
plot.data_adapt <- function(adapt.fit, plot_id = c(1, 2, 3)) {
	...
	if (1 %in% plot_id) {
		...
		plot(top.mean.rank, ylab = 'Mean CV-rank', pch = 20, main = 'Mean CV-rank of selected covariates \n (Smaller the better)')
		library(calibrate)
	...
	if (2 %in% plot_id) {
		...
		plot(sort(p.final), ylab = 'Adjusted P-value', pch = 20, main = 'Adjusted p-value of selected covariates \n (Smaller the better)')
